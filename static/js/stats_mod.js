/**
 * Created by sam.royston on 8/2/15.
 */

var Stats = function (background, foreground) {
	...
	function createPanel( id, fg, bg ) {
		...
		var graph = createElement( 'div', id + 'Graph', 'width:74px;height:30px;background:' + bg );
		div.appendChild( graph );

		for ( var i = 0; i < 74; i ++ ) {

			graph.appendChild( createElement( 'span', '', 'width:1px;height:0px;float:left;opacity:0.9;background:' + fg ) );
	...
	function updateGraph( dom, value ) {
		...
		child.style.height = 1 + 'px';
        child.style.marginTop = Math.min( 30, 30 - value * 30 ) + 'px';
	...
	var fpsDiv = createPanel( 'fps', foreground, background );
	...
	var msDiv = createPanel( 'ms', foreground, background );
	...
	if ( self.performance && self.performance.memory ) {
		...
		var memDiv = createPanel( 'mb', foreground, background );
